using BioBencher;
using BioBencher.Benchmark;
using BioBencher.Config;
using CliWrap;
using CommandLine;
...
public class Program
{
    ...
    static async Task Run(RunOptions options)
    {
        try
        {
            var lines = File.ReadAllLines(options?.FILE);
            if(lines.Length < 1)
            {
                PrintError(OptionsError.UnknownError, "Input FILE has no commands to be benchmarked");
                return;
            }
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                var benchmarkRunner = new BenchmarkRunner(line, options?.WorkingDirectory, CommandFailureAction.Ignore, 0, options.Iterations);
                var run = benchmarkRunner.RunBenchmark(i+1);
